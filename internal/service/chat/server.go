		...
		case client := <-s.Login:
			{
				...
				s.mutex.Unlock()
				zlog.Debug(fmt.Sprintf("欢迎来到lisheng聊天服务器，亲爱的用户%s\n", client.Uuid))
				err := client.Conn.WriteMessage(websocket.TextMessage, []byte("欢迎来到lisheng聊天服务器"))
				if err != nil {
		...
		case data := <-s.Transmit:
			{
				...
				if chatMessageReq.Type == message_type_enum.Text {
					...
					if message.ReceiveId[0] == 'U' { // 发送给User
						...
						// 通过 s.Clients[message.SendId] 获取发送者对应的客户端对象 sendClient
						// Clients 是一个存储所有在线客户端的映射表，键为客户端的唯一标识符（Uuid），值为对应的客户端对象
						sendClient := s.Clients[message.SendId]
						// 将处理后的消息对象 messageBack 通过 sendClient.SendBack 通道发送给发送者
						...
						for _, member := range members {
							// 如果当前成员 member 不是消息的发送者（member != message.SendId），检查该成员是否在线（s.Clients[member] 是否存在）
							if member != message.SendId {
								// 如果在线，则通过其 WebSocket 连接（receiveClient.SendBack）将消息发送给该成员
								if receiveClient, ok := s.Clients[member]; ok {
									receiveClient.SendBack <- messageBack
								}
							} else { // 如果当前成员是消息的发送者，直接通过其 WebSocket 连接（sendClient.SendBack）将消息回显给发送者。
								sendClient := s.Clients[message.SendId]
								sendClient.SendBack <- messageBack
							}
						}
