...
import (
	...
	"LishengChat-go/internal/service/redis"
...
func VerificationCode(telephone string) (string, int) {
	// 创建客户端：调用 createClient 函数创建客户端，若失败返回系统错误
	client, err := createClient()
	if err != nil {
		zlog.Error(err.Error())
		return constants.SYSTEM_ERROR, -1
	}
	// 检查验证码是否存在：从 Redis 中获取指定电话的验证码，若获取失败返回系统错误
	...
	// 验证码过期，重新生成新的6位随机验证码并存储到 Redis 中，有效期为1分钟。
	...
	// 构造短信请求，通过阿里云接口发送验证码短信，若发送失败返回系统错误。
