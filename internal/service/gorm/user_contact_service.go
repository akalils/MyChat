...
import (
	...
	"LishengChat-go/pkg/enum/contact/contact_type_enum"
	"LishengChat-go/pkg/enum/contact_apply/contact_apply_status_enum"

	"LishengChat-go/pkg/enum/group_info/group_status_enum"
	"LishengChat-go/pkg/enum/user_info/user_status_enum"
...
func (u *userContactService) LoadMyJoinedGroup(ownerId string) (string, []respond.LoadMyJoinedGroupRespond, int) {
	...
	if err != nil {
		if errors.Is(err, redis.Nil) {
			...
			return "获取加入群 成功", groupListRsp, 0
		} else {
			zlog.Error(err.Error())
			return constants.SYSTEM_ERROR, nil, -1
		}
...
func (u *userContactService) DeleteContact(ownerId, contactId string) (string, int) {
	...
	// 将UserContact表中ownerId与contactId对应的记录状态改为删除
	...
	// 将UserContact表中contactId与ownerId对应的记录状态改为被删除
	...
	// 更新Session表中与ownerId和contactId相关的会话记录
	...
	// // 删除ContactApply表中与ownerId和contactId相关的申请记录
