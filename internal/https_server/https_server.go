...
import (
	v1 "LishengChat-go/api/v1"
	"LishengChat-go/internal/config"
	"LishengChat-go/pkg/ssl"
...
func init() {
	...
	corsConfig.AllowOrigins = []string{"*"}                                                         // 设置了允许的源列表，表示接受任何源的请求，通常会指定具体的域名
	// corsConfig.AllowOrigins = []string{"http://113.45.13.131"}
	corsConfig.AllowMethods = []string{"GET", "POST", "PUT", "DELETE", "OPTIONS"}                   // 设置允许的HTTP请求方法
	corsConfig.AllowHeaders = []string{"Origin", "Content-Length", "Content-Type", "Authorization", "content-type"} // 设置允许在跨域请求中携带的HTTP头信息
